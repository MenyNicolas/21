...
def partie_BJ():
    ...
    historique_df = tour_BJ(sabot, running_count, historique_df)
...
def tour_BJ(sabot, running_count, historique_df):
    ...
    main_joueur, running_count, sabot, historique_df = jouer_main_joueur(action_stack, main_joueur, main_dealer, running_count, sabot, historique_df, False)
...
def jouer_main_joueur(action_stack, main_joueur, main_dealer, running_count, sabot, historique_df, security_split):

    #log(action_stack, main_joueur, main_dealer, running_count)

    if utils_env.valeur_main(main_joueur) > 21:
        historique_df = utils_env.end_of_hand(main_dealer, main_joueur, action_stack, running_count, sabot, historique_df)
        if not security_split: exit()
    ...
    if utils_env.is_pair(main_joueur):
        if IA_BJ(3, action_stack, main_joueur, main_dealer, running_count, sabot):
            ...
            sabot, running_count, sabot, historique_df = jouer_main_joueur(action_stack.copy(), main_1.copy(), main_dealer, running_count, sabot, historique_df, True)
            sabot, running_count, sabot, historique_df = jouer_main_joueur(action_stack.copy(), main_2.copy(), main_dealer, running_count, sabot, historique_df, False)
    ...
    if action == 'H':
        ...
        sabot, running_count, sabot, historique_df = jouer_main_joueur(action_stack, main_joueur, main_dealer, running_count, sabot, historique_df, security_split)
    if action == 'S':
    ...
    return main_joueur, running_count, sabot, historique_df
